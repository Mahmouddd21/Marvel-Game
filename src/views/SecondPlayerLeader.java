...
public class SecondPlayerLeader {

    public SecondPlayerLeader(JFrame frame) {
		...
		JLabel text = new JLabel(NameSelect.game.getSecondPlayer().getName() +" SELECT YOU LEADER");
        ...
        leader.setBackground(Color.orange);
		...
		for (int i = 0; i < NameSelect.game.getSecondPlayer().getTeam().size(); i++) {
			String name = NameSelect.game.getSecondPlayer().getTeam().get(i).getName();
			...
			Champion champ = NameSelect.game.getSecondPlayer().getTeam().get(i);
			...
			button.addActionListener(e -> {
				...
				if (championLeader.size() == 1) {
					...
					NameSelect.game.getSecondPlayer().setLeader(championLeader.get(0));
				} else {
					nextBTN.setEnabled(false);
				}
		...
		nextBTN.addActionListener(e -> {
			...
			frame.remove(select);
			NameSelect.game = new Game(NameSelect.game.getFirstPlayer(), NameSelect.game.getSecondPlayer());
			new Board(frame, NameSelect.game );
